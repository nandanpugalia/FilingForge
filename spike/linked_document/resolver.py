...
def resolve_document(
    ...
    *,
    ...
    try:
        ...
        return Resolution(status="unresolved", reason=f"unsafe landing resource: {exc}")
    except (httpx.HTTPError, json.JSONDecodeError, KeyError, ValueError) as exc:
        return Resolution(status="unresolved", reason=f"landing resource could not be read: {type(exc).__name__}")

    selected = select_unique_candidate(context, original_evidence.text, choices)
    if selected is None:
        return Resolution(status="unresolved", reason="could not select one unique replacement PDF")
    try:
        validate(selected.url)
    except UnsafeUrl as exc:
        return Resolution(status="unresolved", reason=f"unsafe replacement link: {exc}")
...
def _fetch_and_validate_pdf(context: DocumentContext, url: str, fetch: Fetch) -> Resolution:
    try:
        ...
        return Resolution(status="unresolved", reason=f"replacement download failed: {type(exc).__name__}")
    if not document.body.startswith(b"%PDF-"):
        return Resolution(status="unresolved", reason="replacement is not a PDF")
    try:
        replacement_evidence = extract_pdf_evidence(document.body)
    except (PdfReadError, ValueError, OSError) as exc:
        return Resolution(status="unresolved", reason=f"replacement PDF is unreadable: {type(exc).__name__}")
    if is_linked_cover_letter(context, replacement_evidence):
        return Resolution(status="unresolved", reason="replacement is another cover letter")
    return Resolution(
        status="resolved",
