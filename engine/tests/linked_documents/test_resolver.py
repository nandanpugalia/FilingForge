...
def test_ambiguous_static_candidates_do_not_trigger_pdf_download():
    ...
    assert result.action_url == landing
    assert calls == [(landing, "html")]


def test_cover_without_readable_external_link_has_no_action_url():
    resolver = import_module("engine.linked_documents.resolver")
    fetch, calls = fake_fetch({})
    original = make_pdf(
        "BSE Limited pursuant to Regulation Annual Report FY 2025-26 "
        "is available on the company website at the link below",
        pages=2,
    )

    result = resolver.resolve_document(
        context(), original, fetch=fetch, validate=allow_public
    )
