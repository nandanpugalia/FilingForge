...
- BSE's new website (September 2026) refused every search with "Access Denied", so no
  ...
  own pages send and uses standard secure-connection settings, restoring access on networks
  where headers alone were insufficient. Certificate verification and custom certificate
  settings are preserved. Searches, announcements and downloads work again.
